...
<html>
	...
	<body onload="addActiveToCash()">
		<div id="T_11_POS_Payment-rev1">
			...
			<div id="Group_2141">
				<a  href="login.php?login=<?php echo $_GET['login'];?>" id="Group_2090">
...
$cus=$_POST['cus'];
		  ...
		  $_SESSIO['cus']=$cus;
			         echo "<script>document.location='billing.php?login=$com&cus=$cus'</script>";   
...
<center>
      ...
      <select style="width:100%" class="selectpicker" name="cus"  data-show-subtext="true" data-live-search="true" required>
...
if($rowuser['account_type']=="Distributor"){
$query=mysqli_query($con,"select * from  user where account_type='Distributor' and region='$region' order by firstname asc")or die(mysqli_error());
...
$count++;
        ...
        <option value="<?php echo $row['id'];?>"><?php echo $row['id_number'];?> | <?php echo $row['firstname'];?> <?php echo $row['lastname'];?></option>
...
$querys=mysqli_query($con,"select * from  my_distri where distri='$com'")or die(mysqli_error());
while($rows=mysqli_fetch_array($querys)){  
$user_id=$rows['user_id'];
$query=mysqli_query($con,"select * from  user where id='$user_id'")or die(mysqli_error());
...
$com=$_GET['login'];
$region=$rowuser['region'];
$query=mysqli_query($con,"select * from  user where account_type='Dealer' and region='$region' order by firstname asc")or die(mysqli_error());
while($row=mysqli_fetch_array($query)){  
...
$count++;
  ...
  <script src="//cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.6.3/js/bootstrap-select.min.js"></script>
						...
						<div class="row mt-3 d-block text-center">
							<button type="submit" name="signin" class="charge-amount">Select</button>
						</div>
</form>
					</div>

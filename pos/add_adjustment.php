...
<html>
	...
	<body>
		...
		<div id="T_Products_to_Adjust">
			...
			<a  href="adjustment.php?login=<?php echo $_GET['login'];?>" id="Group_2090">
...
if(isset($_POST['signin']))
{
               ...
               $com=$_GET['login'];           
	$query21a1=mysqli_query($con,"select * from  adjustment where store_id='$com'")or die(mysqli_error());
...
$date=date("Y-m-d H:i");
 ...
 mysqli_query($con,"INSERT INTO adjustment(store_id,prod_id,type,qty,remarks,sales_agent,reason,code,date_added,status) VALUES('$ses','$prod_id','$action','$quantity','$remarks','$sales_agent','$reason','$number','$date','Pending')")or die(mysqli_error($con));
...
?>
    ...
    window.setTimeout(function() {
       window.location.href='adjustment.php?login='<?php echo $_GET['login'];?>;
    }, 1000);
</script>
...
swal("Success!", "Adjustment Successfully Requested!", "success");
    ...
    ?>
				...
				<div class="row mt-3 ">
					...
					<div class="col-12 col-sm-6 col-lg-6">
						...
						<div class="row reverse-order">
							...
							<div class="col-12 col-sm-8 col-lg-8">
								<button type="submit" name="signin" class="request-adjustment">Request Adjustment</button>
							</div>
						</div>
		...
		<script type="text/javascript">
			...
			function new_row() {
				...
				div1.id = ct;
				var delLink ='<div class="per_product_return_container" id="'+ct+'"><div class="row"><div class="col-10 col-sm-3 col-lg-3 mt-xs-1 pr-sm-0"><select id="name'+ct+'" class="select2" name="name[]" required><option value="" disabled selected hidden>Choose Product</option><?php $query=mysqli_query($con,"select * from  products where company=0")or die(mysqli_error()); while($row=mysqli_fetch_array($query)){  ?><option value="<?php echo $row['id'];?>"><?php echo $row['prod_no'];?>|<?php echo $row['prod_name'];?></option><?php } ?></select></div><div class="col-2 col-sm-1 col-lg-1 mt-xs-1 mobile-view"><div class="wrapper text-center"><a class="removeBtn" href="javascript:delIt('+ ct +')"><i class="fa fa-trash-alt"></i></a></div></div><div class="col-12 col-sm-2 col-lg-2 mt-xs-1 pr-sm-0"><input type="text" id="quantity'+ct+'" name="quantity[]" placeholder="Quantity"></div><div class="col-12 col-sm-2 col-lg-2 mt-xs-1 pr-sm-0"><select id="action'+ct+'" name="action[]" required><option value="" disabled selected hidden>Action</option><option value="Increase (+)">Increase (+)</option><option value="Decrease (-)">Decrease (-)</option></select></div><div class="col-12 col-sm-4 col-lg-4 pr-sm-0 pr-lg-0 mt-xs-1"><input type="text" id="reason'+ct+'" name="reason[]" placeholder="Type your reason here"></div><div class="col-1 pl-sm-p25 pr-sm-0 pr-lg-0 big-screen-view"><div class="wrapper text-center"><a class="removeBtn" href="javascript:delIt('+ ct +')"><i class="fa fa-trash-alt"></i></a></div></div></div></div>'

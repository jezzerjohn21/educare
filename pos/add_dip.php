	...
	<body>
		...
		<div id="T_Products_to_Adjust">
			...
			<div id="Select_Products_to_Adjust">
				<span>Dealer/Sales Clerk Incentive Program</span>
			</div>
			<a  href="dip.php?login=<?php echo $_GET['login'];?>" id="Group_2090">
...
if(isset($_POST['signin']))
{
               ...
               $com=$_GET['login'];           
	$query21a1=mysqli_query($con,"select DISTINCT code from  dip ")or die(mysqli_error());
...
$remarkssss=$_POST['remarks22'][$i];
$quantity=$_POST['target'][$i];
$action=$_POST['packaging'][$i];
$reason=$_POST['budget'][$i];
$remarks=$_POST['incentive'][$i];
$ses=$_GET['login'];
...
$date=date("Y-m-d H:i");
 ...
 mysqli_query($con,"INSERT INTO dip(user_id,product_id,packaging,target,budget,incentive,remarks,code,date_added,status) VALUES('$ses','$prod_id','$action','$quantity','$reason','$remarks','$remarkssss','$number','$date','Pending')")or die(mysqli_error($con));
...
?>
    ...
    window.setTimeout(function() {
       window.location.href='dip.php?login='<?php echo $_GET['login'];?>;
    }, 1000);
</script>
...
?>           
			...
			<div class="products_adjust_container">
				...
				<div class="row mb-2">
					...
					<div class="col-12 col-sm-12 col-lg-12">
						<input type="text" id="remarks" name="remarks22" placeholder="Remarks" required>
					</div>
  ...
  <br>
  &nbsp;&nbsp;&nbsp;Greetings from Leads Agri. Thank you for continued patronage of our products!
  </label>			</div>
					...
					<div class="col-12 col-sm-6 col-lg-6">
						...
						<div class="row reverse-order">
							...
							<div class="col-12 col-sm-8 col-lg-8">
								<button type="submit" name="signin" class="request-adjustment">Submit</button>
							</div>
						</div>
  ...
  <br>
  &nbsp;&nbsp;&nbsp;1. Only purchases from accredited distributors will considered in this incentive Programs.
  </label>			</div>
			</div>
		...
		<script type="text/javascript">
			...
			function new_row() {
				...
				div1.id = ct;
				var delLink ='<div class="per_product_return_container" id="'+ct+'"><div class="row"><div class="col-10 col-sm-3 col-lg-3 mt-xs-1 pr-sm-0"><select id="name'+ct+'" class="select2" name="name[]" required><option value="" disabled selected hidden>Choose Product</option><?php $query=mysqli_query($con,"select * from  products where company=0")or die(mysqli_error()); while($row=mysqli_fetch_array($query)){  ?><option value="<?php echo $row['id'];?>"><?php echo $row['prod_no'];?>|<?php echo $row['prod_name'];?></option><?php } ?></select></div><div class="col-2 col-sm-1 col-lg-1 mt-xs-1 mobile-view"><div class="wrapper text-center"><a class="removeBtn" href="javascript:delIt('+ ct +')"><i class="fa fa-trash-alt"></i></a></div></div><div class="col-12 col-sm-2 col-lg-2 mt-xs-1 pr-sm-0"><input type="text" id="quantity'+ct+'" name="packaging[]" placeholder="Packaging"></div><div class="col-12 col-sm-2 col-lg-2 mt-xs-1 pr-sm-0"><input type="text" id="quantity'+ct+'" name="target[]" placeholder="Target"></div><div class="col-12 col-sm-2 col-lg-2 pr-sm-0 pr-lg-0 mt-xs-1"><input type="text" id="reason'+ct+'" name="budget[]" placeholder="Budget"></div><div class="col-12 col-sm-2 col-lg-2 pr-sm-0 pr-lg-0 mt-xs-1"><input type="text" id="reason'+ct+'" name="incentive[]" placeholder="Incentive"></div><div class="col-1 pl-sm-p25 pr-sm-0 pr-lg-0 big-screen-view"><div class="wrapper text-center"><a class="removeBtn" href="javascript:delIt('+ ct +')"><i class="fa fa-trash-alt"></i></a></div></div></div></div>'
